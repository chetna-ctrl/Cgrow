...
const VPDWidget = ({ initialTemp, initialHumidity, compact = false }) => {
        ...
        return (
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-bold text-slate-700 text-sm">VPD Calculator</span>
                </div>
    ...
    // Full widget
    return (
        <div className="bg-gradient-to-br from-emerald-50 to-cyan-50 p-6 rounded-2xl border-2 border-emerald-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="font-bold text-slate-800">VPD Calculator</h3>
                        <p className="text-xs text-slate-500">Vapor Pressure Deficit</p>
                    </div>
                </div>
                <button
            ...
            {showInfo && (
                <div className="bg-white/70 p-4 rounded-xl mb-4 text-sm text-slate-600">
                    <p className="font-bold mb-2">What is VPD?</p>
                    <p className="mb-2">
                        VPD measures how "thirsty" the air is. It's the difference between how much
            ...
            <div className={`p-5 rounded-xl transition-all ${vpd === null ? 'bg-slate-100 border-2 border-dashed border-slate-300' :
                ...
                {vpd === null ? (
                    <p className="text-center text-slate-400">
                        Enter temperature & humidity to calculate VPD
                    </p>
                ) : (
                    <>
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-3xl font-bold">{vpd} kPa</span>
                            <span className="text-4xl">{vpdInfo.icon}</span>
                        </div>
                        <p className="font-bold mb-1">{vpdInfo.label}</p>
                        <p className="text-sm opacity-90">{vpdInfo.recommendation}</p>
                    </>
                )}
            </div>
